...
return function(shared, page)
    ...
    -- [[ HTTPSLOG: Save HTTP requests to file ]] --
    local LOG_FILE = "httpslog.txt"
    local http_log_env = getfenv()
    local function saveHttpLog(url, method, func, headers, body)
        pcall(function()
            local wf = http_log_env.appendfile or http_log_env.writefile
            if not wf then return end

            local timestamp = os.date and os.date("%Y-%m-%d %H:%M:%S") or tostring(tick())
            local headerStr = "nil"
            if type(headers) == "table" then
                ...
                headerStr = table.concat(parts, ", ")
            end
            local bodyStr = type(body) == "string" and body or "nil"

            local entry = string.format(
                "[%s] %s %s | Func: %s | Headers: {%s} | Body: %s\n",
                timestamp,
                tostring(method or "GET"),
                tostring(url),
                tostring(func or "request"),
                headerStr,
                bodyStr
            )

            if http_log_env.appendfile then
                http_log_env.appendfile(LOG_FILE, entry)
            else
                -- fallback: read existing + write combined
                local existing = ""
                pcall(function()
                    existing = http_log_env.readfile(LOG_FILE) or ""
                end)
                http_log_env.writefile(LOG_FILE, existing .. entry)
            end
        end)
    end
    -- [[ END HTTPSLOG ]] --
    ...
    local function addLog(realUrl, method, func, headers, body, result)
        ...
        -- Save to httpslog.txt (no GUI)
        saveHttpLog(realUrl, method, func, headers, body)
